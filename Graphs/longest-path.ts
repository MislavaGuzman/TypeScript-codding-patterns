...
function dfs(r: number, c: number, matrix: number[][], memo: number[][]): number {
  ...
  for (const [dr, dc] of dirs) {
    ...
    if (isWithinBounds(nextR, nextC, matrix) && matrix[nextR][nextC] === matrix[r][c] + 1) {
...
console.log(longestConsecutivePath(matrix1));
...
console.log(longestConsecutivePath(matrix2));
